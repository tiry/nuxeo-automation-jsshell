...
(function(jQuery, nuxeo) {
  ...
  function nxshell(opts) {
    ...
    var initBuiltIns = function() {
      ...
      for ( var key in builtInsDefs) {
        ...
        var builtin = function(k) {
          return function(cmds, term) {
            return builtInsDefs[k].impl(cmds, term, shell)
          }
        }(key);
    ...
    nxshell.prototype.displayNavigationPrompt = function(term, prevPageCB,
        nextPageCB) {
      term
          .set_prompt("( use arrows to navigate between pages - q to quit )");
    ...
    nxshell.prototype.displayPagesFromList = function(term, list, pageSize,
        offset) {

      for ( var idx = offset; (idx < list.length)
          && (idx < offset + pageSize); idx++) {
        term.echo(list[idx]);
      }
      ...
      var nextCB = function() {
        term.pop();
        me
            .displayPagesFromList(term, list, pageSize, offset
                + pageSize);
      }
      var prevCB = function() {
        term.pop();
    ...
    nxshell.prototype.displayPagesFromOperation = function(term, operation,
      ...
      var doDisplayPage = function(docs, term) {
        ...
        cb.push(function() {
          ...
          var prevCB = function() {
            term.pop();
            fetchPage(prevIdx, term)
          };
          var nextCB = function() {
            term.pop();
            fetchPage(nextIdx, term)
          };
          ...
          term.echo("  [ display page : " + (docs.pageIndex + 1)
              + "/" + docs.pageCount + "]");
          me.displayNavigationPrompt(term, prevCB, nextCB);
        })
      ...
      var fetchPage = function(page, term) {
        ...
        operation.done(function(data, status, xhr) {
          successCB(data, status, xhr, term)
        }).fail(function(xhr, status) {
          errorCB(xhr, status, term);
        }).execute();
      }
      fetchPage(0, term);
    ...
    nxshell.prototype.displayHelp = function(cmd, term) {
      if (cmd === undefined) {
        term
            .echo("use help 'commandName' to get help on the command or operation named 'commandName'");
        ...
        for ( var idx = 0; idx < this.builtins.length; idx++) {
          term.echo("[[b;#00EE00;#0]" + this.builtins[idx].id
              + "] : [[i;#CCCCCC;#0]" + this.builtins[idx].help
              + "]");
        }
      } else if (cmd == 'ops') {
        var lines = [];
        for ( var idx = 0; idx < this.automationDefs.operations.length; idx++) {
          lines.push("[[b;#00EE00;#0]"
              + this.automationDefs.operations[idx].id
              + "] : [[i;#CCCCCC;#0]"
              + this.automationDefs.operations[idx].label + "]");
        }
    ...
    nxshell.prototype.printDoc = function(doc, relPath) {
      ...
      return "  [[bi;#8888FF;#0] {doc} ] " + path + " '" + title + "' "
          + " (" + doc.type + ") [[i;#AAAAAA;#0] " + doc.uid + " ]";
    }
    ...
    nxshell.prototype.initialCommandCompletion = function(term, existingInput, callback) {
      var suggestions = [];
      var initialInput = existingInput.split(" ")[0];
      console.log("initialCommandCompletion : ", initialInput);
      var op = this.findOperation(initialInput);
      if (op) {
        for ( var idx = 0; idx < op.params.length; idx++) {
          var pName = op.params[idx].name;
          // skip parameters that already have a value
        ...
        for ( var idx = 0; idx < this.builtins.length; idx++) {
          ...
          if (cmdOp.id.indexOf(initialInput) == 0) {
        ...
        for ( var idx = 0; idx < this.automationDefs.operations.length; idx++) {
          var op = this.automationDefs.operations[idx];
          if (op.id.indexOf(initialInput) == 0) {
    ...
    var suggesters = {
      nxql : function(term, totalInput, value, callback, shell) {
        var query = term.get_command().trim();
        var suggestions = [];
        if (query.indexOf("|") > 0) {
          var pipeCmd = query.split("|")[1].trim();
          shell.initialCommandCompletion(term, pipeCmd, callback);
          return;
        }
        if (query.indexOf("*") < 0) {
          suggestions.push("*");
      ...
      path : function(term, totalInput, value, callback) {
        ...
        var parentPath = absolutePath.substring(0, absolutePath
            .lastIndexOf("/") + 1);
        var name = absolutePath
            .substring(absolutePath.lastIndexOf("/") + 1);

        var query = "select * from Document where ecm:path STARTSWITH '"
            + parentPath + "' AND ecm:isCheckedInVersion= 0";
        if (name.length > 0) {
            ...
            .done(
                function(docs, status, xhr) {
                  if (docs.entries.length > 9) {
                    term
                        .echo("... too much results to complete ...");
                    return;
                  }
                  for ( var i = 0; i < docs.entries.length; i++) {
                    if (value.indexOf("/") != 0) {
                      suggestions
                          .push(docs.entries[i].path
                              .substring(shell.ctx.path.length + 1));
                    } else {
                      suggestions
                          .push(docs.entries[i].path);
                    }
    ...
    nxshell.prototype.cmdParamCompletion = function(term, cmd, input,
        callback) {

      if (Object.prototype.toString.call(cmd.suggest) === '[object Array]'
          && cmd.suggest.length > 0) {
      ...
      } else if (typeof (cmd.suggest) == 'string'
          && cmd.suggest.length > 0) {
        suggesters[cmd.suggest](term, input, value, callback, this);
      }
    }
    ...
    nxshell.prototype.completion = function(term, input, callback) {
      ...
      if (cmd) {
        ...
        this.initialCommandCompletion(term, existingInput, callback);
      }
    }
    ...
    nxshell.prototype.init = function(term) {
      ...
      nuxeo.operation('Document.Fetch', {
        automationParams : {
          params : {
            value : root
          }
          ...
          function(data, status, xhr) {
            ...
            term.echo("Connected on repository '" + data.repository
                + "'");
            term.echo(" current path is " + data.path + "("
                + data.uid + ")");
          }).fail(function(xhr, status) {
        console.log("Error " + status);
      }).execute();
  ...
  nuxeo.shell = function(filter, opts) {
    ...
    opts.greetings = function() {
      return nx.nxGreetings()
    };
    opts.completion = function(term, input, callback) {
      return nx.completion(term, input, callback)
    };
    ...
    if (filter) {
      ...
      function mkButton(label, cb, img) {
        ...
        if (img) {
          var icon = jQuery('<img src="' + '/nuxeo/icons/shell/'
              + img + '" title="' + label + '"/>');
          btn = jQuery("<div></div>");
          btn.append(icon);
        }
      ...
      var smallScreenBtn = mkButton("Small terminal", function(event,
          term) {
